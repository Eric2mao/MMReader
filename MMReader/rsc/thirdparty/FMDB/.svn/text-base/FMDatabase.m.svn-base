...
- (BOOL)open {
    ...
    if(err != SQLITE_OK) {
        CommonDatabaseLog_i(@"error opening!: %d", err);
        
#ifdef MTT_ENABLE_QBDEBUG
        [AnalyticsInterface onUserAction:@"fmdbOpenFailed" isSucceed:NO elapse:0 size:0 params:@{@"param_FailCode": @(err), @"dbPath": _databasePath }];
#endif
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                CommonDatabaseLog_i(@"%s:%d", __FUNCTION__, __LINE__);
                CommonDatabaseLog_i(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, 0x00)) !=0) {
                    CommonDatabaseLog_i(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            CommonDatabaseLog_i(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        CommonDatabaseLog_i(@"error on rekey: %d", rc);
        CommonDatabaseLog_i(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    CommonDatabaseLog_i(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        CommonDatabaseLog_i(@"The FMDatabase %@ is not open.", self);
    ...
    for (NSUInteger i = 0; i < length; ++i) {
        ...
        if (last == '%') {
            switch (current) {
                ...
                case 'D':
                case 'i':
                    arg = [NSNumber numberWithInteger:va_arg(args, int)];
                    break;
                case 'u':
                case 'U':
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        CommonDatabaseLog_i(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                    CommonDatabaseLog_i(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    CommonDatabaseLog_i(@"Database busy");
                    sqlite3_finalize(pStmt);
                    _isExecutingStatement = NO;
                    return nil;
                }
                ...
                if (_logsErrors) {
                    CommonDatabaseLog_i(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    CommonDatabaseLog_i(@"DB Query: %@", sql);
                    CommonDatabaseLog_i(@"DB Path: %@", _databasePath);
#ifndef NS_BLOCK_ASSERTIONS
                    if (_crashOnErrors) {
                        abort();
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                CommonDatabaseLog_i(@"Could not find index for %@", dictionaryKey);
            }
        }
    ...
    if (idx != queryCount) {
        CommonDatabaseLog_i(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (FMResultSet *)executeQuery:(NSString*)sql, ... {
    ...
    result = [self executeQuery:sql withArgumentsInArray:nil orDictionary:nil orVAList:args];
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        CommonDatabaseLog_i(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        CommonDatabaseLog_i(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    CommonDatabaseLog_i(@"Unexpected result from sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(20);
            ...
            CommonDatabaseLog_i(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            CommonDatabaseLog_i(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            CommonDatabaseLog_i(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            CommonDatabaseLog_i(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            CommonDatabaseLog_i(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            CommonDatabaseLog_i(@"DB Query: %@", sql);
        }
    ...
    if (closeErrorCode != SQLITE_OK) {
        CommonDatabaseLog_i(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
        CommonDatabaseLog_i(@"DB Query: %@", sql);
    }
...
- (BOOL)executeUpdate:(NSString*)sql, ... {
    ...
    result = [self executeUpdate:sql error:nil withArgumentsInArray:nil orDictionary:nil orVAList:args];
...
@implementation FMDatabase (FMDatabaseMttExtension)

- (NSString *)dbVersion
{
	if([self goodConnection])
	{
		if([self tableExists:@"meta"])
		{
			// read time
			NSString* ver = nil;
			NSString* sql = @"select * from meta";
			FMResultSet* result = [self executeQuery:sql];
			while([result next])
			{
				ver = [result stringForColumnIndex:1];
			}
			[result close];
			return ver;
		}
	...
	if([self goodConnection])
	{
		if([self tableExists:@"meta"])
		{
			...
			while([result next])
			{
				...
				//				CommonDatabaseLog_i(@"dd,read date:%@",date);
			}
			[result close];
			return date;			
		}
...
- (BOOL)setTimestamp:(NSDate *)time
{
	if([self goodConnection] && [self tableExists:@"meta"])
	{
		NSString* sql = @"UPDATE meta SET timestamp = ?";
		return [self executeUpdate:sql,time];
	}
	else
	{
		return NO;
