...
- (void)setupColumnNames {
    ...
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
        [_columnNameToIndexMap setObject:[NSNumber numberWithInteger:columnIdx]
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        CommonDatabaseLog_w(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    CommonDatabaseLog_e(@"Unexpected result from sqlite3_reset (%d) rs", rc);
                }
            }
            usleep(20);
            ...
            if ([_parentDB busyRetryTimeout] && (numberOfRetries++ > [_parentDB busyRetryTimeout])) {
                ...
                CommonDatabaseLog_e(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
                CommonDatabaseLog_e(@"Database busy");
                break;
            }
        }
            ...
            CommonDatabaseLog_e(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            CommonDatabaseLog_e(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    CommonDatabaseLog_w(@"Warning: I could not find the column named '%@'.", columnName);
