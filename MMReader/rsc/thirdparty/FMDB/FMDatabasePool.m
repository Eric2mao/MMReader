...
- (FMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            if (_maximumNumberOfDatabasesToCreate) {
                ...
                if (currentCount >= _maximumNumberOfDatabasesToCreate) {
                    CommonDatabaseLog_i(@"Maximum number of databases (%ld) has already been reached!", (long)currentCount);
                    return;
                }
            }
        ...
        if ([db open]) {
            ...
            CommonDatabaseLog_i(@"Could not open up the database at path %@", _path);
            db = 0x00;
        }
    }];
